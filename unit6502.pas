...
var instructions:int64=0; //keep track of total instructions executed
    clockticks6502:int64=0;
    clockgoal6502:int64=0;

function read6502(address:int64):byte;
procedure write6502(address:int64; value:byte);
procedure reset6502;
procedure nmi6502;
procedure irq6502;
procedure exec6502(tickcount:int64);
procedure fast6502(tickcount:int64);
procedure step6502;
procedure jsr6502(aa:word; addr:int64);
...
var
    ...
    csa,dsa,csi,dsi:int64;
    cs,ds:^int64;

//These functions has to be implemented outside this unit

function read6502(address:int64):byte;

begin
result:=ram[address and $FFFF];
end;

procedure write6502(address:int64; value:byte);

begin
ram[address and $FFFF]:=value;
end;
...
//  if instructions>3000 then box(100,100,100,100,44) else box(100,100,100,100,198);
end;
...
ea += x;
if (startpage <> (ea and $FF00)) then penaltyaddr := 1;   //one cycle penlty for page-crossing on some opcodes
pc += 2;
end;
...
//ea := word(read6502(eahelp)) or (word(read6502(eahelp+1)) shl 8);
